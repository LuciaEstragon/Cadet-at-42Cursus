...
char	*get_next_line(int file)
{
	...
	{
		free(save);
		save = NULL;  //importante! al ser una variable static siempre existe, asi que aunque la liberes, el puntero que apunta al espacio que ocupa siempre existe, aunque cierres la funcion.
		return (NULL);
	}
	line = make_line(&save);
	return (line);
