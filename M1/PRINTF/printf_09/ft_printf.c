...
/*   Updated: 2026/01/09 12:06:44 by lestrada         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*static char	wrapper_upper(unsigned int i, char c)
{
	(void)i;
...
static int	function_types(char const *s, va_list arg)
{
	...
	if (*s == 'd' || *s == 'i')
		...
		//return (ft_putstr_free(ft_itoa_base(va_arg(arg, int), 10))); --EL ANTIGUO ITOA PEDIA LA BASE COMO ENTERO )base¿10=, AHORA COMO DIGITOS
	...
	if (*s == 'X'){
		...
		char *str_mapi = ft_strmapi(str_itoa, wrapper_upper);
		free(str_itoa);
		return (ft_putstr_free(str_mapi)); ''FUNCIONA PERO ES LARGUISIMO */
		return (ft_putstr_free(ft_itoa_base(va_arg(arg, int), "0123456789ABCDEF")));
	}
	if (*s == 'p')
	{
		write(1, "0x", 2);
		return (ft_putstr_free(ft_itoa_base(va_arg(arg, int), "0123456789abcdef")) + 2);
	}
	return (0);
}
