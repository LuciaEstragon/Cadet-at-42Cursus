...
/*   Updated: 2025/12/21 18:43:08 by lestrada         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*make_line(char **save)
{
	...
	printf("SAVE_aux %s\n", save_aux);
	printf("SAVE %s\n", *save);
	line = ft_substr(*save, 0, (int)ft_strlen(*save)-(int)ft_strlen(save_aux)+1); // para qu te coja \n en la linea le pongo un +1 para que termine uno mas tarde
	printf("LINE %s\n", line);
	*save = ft_substr(save_aux, 1, (int)ft_strlen(save_aux)); //para que no te ccooja el \n en la linea le pongo que empiece desde +1
	printf("SAVE %s\n", *save);
	return (line);
}
