...
class Maze(ConfigParams):
    ...
    def generate_maze(self):
        ...
        while cola:
            ...
            print(f"Casilla ({row},{col}) = {self.maze[row][col]}")
            ...
            '''
            if not cola and any(self.maze[row][col] in ('·', '*') for row in range(self.height) for col in range(self.width)):
                print("== PRINT ==")
                printable_maze = ""
                ...
                break_wall(row, col, self.maze, self.height, self.width)
                cola.append((a, b))
            '''
            if not cola and any(self.maze[i][j] in ('·', '*') for i in range(self.height) for j in range(self.width)):
                ...
                if coords:
                    cola.append(coords)
                    print(f"Reanudando desde {coords}")
