...
static int	function_types(char const *s, va_list arg)
{
	...
	if (*s == 'x')
		return (ft_free_putstr(ft_itoa_base(va_arg(arg, int), HEX_DOWN)));
	if (*s == 'X')
		return (ft_free_putstr(ft_itoa_base(va_arg(arg, int), HEX_UP)));
	if (*s == 'p')
	{
		...
		write(1, "0x", 2);
		return (ft_free_putstr(ft_itoa_base_ptr(ptr, HEX_DOWN)) + 2);
	}
	return (0);
}
